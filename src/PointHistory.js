...
const PointHistory = () => {
  ...
  const shopItems = [
    {
      ...
      name: '셔틀버스 1회 이용권',
      price: 1000,
      imageUri: require('../assets/1000won.png'),
    },
    {
      id: '2',
      name: '코나킹 3000원 쿠폰',
      price: 3000,
      imageUri: require('../assets/konaking.png'),
    },
    {
      id: '3',
      name: 'CU 3000원 기프티콘',
      price: 3000,
      imageUri: require('../assets/CU.png'),
    },
    {
      id: '4',
      name: 'CU 5000원 기프티콘',
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userDoc.exists) {
          ...
          // 포인트 로그 하위 컬렉션 가져오기
          const pointLogsCollection = await firebase
            .firestore()
            .collection('users')
            .doc(firebase.auth().currentUser.uid)
      ...
      Alert.alert('알림', '쿠폰을 구매하였습니다.', [
        {
          text: '확인',
          onPress: () => {
            navigation.navigate('Profile', { couponname });
          },
        },
      ]);
  ...
  const renderBodyContent = () => {
    if (selectedIndex === 0) {
      ...
      const sortedPointLogs = pointLogs
        .slice()
        ...
        <View style={styles.middleview}>
          <TouchableOpacity onPress={() => setSelectedIndex(0)}>
            <Text
              style={[
                styles.headertext2,
                selectedIndex === 0 && styles.headertext3,
              ]}
            >
              포인트 적립 내역
            </Text>
          ...
          <TouchableOpacity onPress={() => setSelectedIndex(1)}>
            <Text
              style={[
                styles.headertext2,
                selectedIndex === 1 && styles.headertext3,
              ]}
            >
              포인트 상점
            </Text>
