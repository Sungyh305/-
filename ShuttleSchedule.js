...
const ShuttleSchedule = ({ navigation }) => {
  ...
  const renderScheduleItem = ({ item }) => {
    ...
    const onPressHandler =
      item.time !== 'X'
        ? () => navigation.navigate('TrainSchedule', { scheduleItem: item })
        : null;
    ...
    return (
      ...
      >
        ...
        {(selectedStation === 2 || selectedStation === 3) && (
          <Text style={textStyle}>{item.approximately}</Text>
        )}
        <Text style={textStyle}>{item.arrivalTime}</Text>
        {selectedStation === 3 && (
          <>
            <Text style={textStyle}>{item.approximately2 || ''}</Text>
            <Text style={textStyle}>{item.time2 || ''}</Text>
            <Text style={textStyle}>{item.arrivalTime2 || ''}</Text>
          </>
        )}
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity
        style={styles.stationSelectButton}
        onPress={handleStationSelect}
      >
        <Text style={styles.stationSelectButtonText}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => handleStationChange(item)}
                >
